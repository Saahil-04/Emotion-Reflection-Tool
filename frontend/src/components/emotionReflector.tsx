...
export function EmotionReflector() {
  const [text, setText] = useState("");
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<{ emotion: string; confidence: number; response: string } | null>(null);
  ...
  const handleSubmit = async () => {
    ...
    setError("");
    try {
      const res = await fetch(`http://localhost:8000/reflect`, {
        method: "POST",
